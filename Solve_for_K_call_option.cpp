...
{
    double N_d1 = SimpsonsDistribution(d_1(t, S, x, T, v, q, r));
    double N_d2 = SimpsonsDistribution(d_2(t, S, x, T, v, q, r));
    double temp = ((S * (exp(-q*T) * (N_d1))) - (x * exp(-r*T) * N_d2)) - S + x;
    return temp;
}
...
double d_call_interms_K(double t, double S, double K_old, double T, double x, double q, double r) {
    //  double N_d1 = SimpsonsDistribution(d_1(t, S, K_old, T, x, q, r));
    double N_d2 = SimpsonsDistribution(d_2(t, S, K_old, T, x, q, r));
...
int main() {
    ...
    double v = 0.25; // sigma, volitility
    
    double S = 40.0; // spot price
    double q = 0.02; // dividends
    
    double r = 0.03; // interest rate
    
    double T = 0.75; // End time of option in terms of Year 1.0
    
    
    double x_0 = 40.0; // initial guess strike price
    ...
    {
        cout << std::setprecision(11)  << x_new << endl;
